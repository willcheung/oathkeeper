...
public class MSGraphEventProducerTest {

    @Test
    public void testAccess() throws Exception {
        String token = getAccessToken();
        IGraphServiceClient client = new Office365ServiceProvider().createClient(token);
        MSGraphEventProducer producer = new MSGraphEventProducer(client);
        //producer.maxMessages(10);
        List<Event> events = producer.asFlux()./*doOnNext(ev -> System.out.println("" + ev.getSummary() + " : " + ev.getStart())).*/collectList().block();
