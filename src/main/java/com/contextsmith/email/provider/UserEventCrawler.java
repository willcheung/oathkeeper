...
import com.contextsmith.email.provider.office365.MSGraphEventProducer;
...
public class UserEventCrawler {
    ...
    public void addGmailTask(Function<String, GoogleServiceProvider> googleServiceProvider, final String accessToken, final String email,
    ...
    public void addExchangeTask(Supplier<ExchangeServiceProvider> exchangeServiceProvider, Source source, final long startDate, final long endDate, int maxEvents) {
        this.callables.add(() -> {
            ...
            return produceEvents(source, producer.asFlux());
        });
    }

    public void addOffice365Task(Function<String, IGraphServiceClient> office365Provider, Source source, final long startDate, final long endDate, int maxEvents) {
        this.callables.add(() -> {
            IGraphServiceClient client = office365Provider.apply(source.token);
            MSGraphEventProducer producer = new MSGraphEventProducer(client).maxMessages(maxEvents).startDate(startDate).endDate(endDate);
            return produceEvents(source, producer.asFlux());
        });
    }

    /**
     * Measure flux and time for producing events from Exchange or Outlook.
     */
    private List<Event> produceEvents(Source source, Flux<Event> eventFlux) {
        Runtime runtime = Runtime.getRuntime();
        long usedMemoryBefore = runtime.totalMemory() - runtime.freeMemory();
        System.out.println("Used Memory before: " + usedMemoryBefore / 1_000_000);
        long startTime = System.currentTimeMillis();

        List<Event> events = eventFlux.map(event -> {
            @SuppressWarnings("unchecked")
